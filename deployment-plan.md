...
### Step 1: Add `vercel.json` to Project
A `vercel.json` file in the root coordinates rewrites so that frontend requests to `/api/*` are transparently proxied to Railway without CORS issues.
...
4. Configure Project Settings:
   ...
   - **Build Command:** *(Leave empty / not required for vanilla HTML/JS)*
   - **Output Directory:** *(Leave empty / defaults to root)*

### Step 3: Set Environment Variables on Vercel (Optional)
If deploying standalone without rewrites, add an Environment Variable in the Vercel dashboard:
- `NEXT_PUBLIC_API_URL` or `BACKEND_URL`: `https://<your-railway-url>.up.railway.app`
